...
const CACHE_NAME = 'waveforge-cache-v1';
...
self.addEventListener('install', (event) => {
    console.log('[SW] Installing - caching assets for offline use');
    event.waitUntil(
    ...
    event.respondWith(
        caches.match(event.request).then(cachedResponse => {
            ...
            return fetch(event.request).then(response => {
                ...
                // Network failed - try to serve a cached version
                return caches.match('/');
            });
        })
    );
